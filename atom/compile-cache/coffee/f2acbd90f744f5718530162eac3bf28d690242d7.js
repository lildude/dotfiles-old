(function() {
  var Commands, HEADING_REGEX, LIST_OL_REGEX, LIST_TL_REGEX, LIST_UL_REGEX, REFERENCE_REGEX, TABLE_COL_REGEX, TABLE_VAL_REGEX, config, utils;
  ...
  Commands = (function() {
    ...
    Commands.prototype.formatTable = function() {
      var editor, lines, options, range, rows, table, _ref;
      editor = atom.workspace.getActiveTextEditor();
      lines = this._getSelectedLines(editor);
      ...
      table = this._createTable(rows, options);
    ...
    Commands.prototype._parseTable = function(lines) {
      var alignments, columnWidth, columnWidths, extraPipes, i, line, numOfColumns, row, rows, separator, _i, _j, _k, _len, _len1, _len2, _ref;
      rows = [];
      numOfColumns = 0;
    ...
    Commands.prototype._createTable = function(rows, options) {
      var row, table, _i, _len, _ref;
      table = [];
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        row = _ref[_i];
        table.push(utils.createTableRow(row, options));
      }
      return table.join("\n");
    };
    ...
    Commands.prototype.createDefaultKeymaps = function() {
      var fs, path, sampleKeymap, sampleKeymapFile, userKeymapFile;
      fs = require("fs-plus");
      path = require("path");
      sampleKeymapFile = utils.getPackagePath("keymaps", this._sampleKeymapFile());
      sampleKeymap = fs.readFileSync(sampleKeymapFile);
      userKeymapFile = path.join(atom.getConfigDirPath(), "keymap.cson");
      return fs.appendFile(userKeymapFile, sampleKeymap, function(err) {
        if (!err) {
    ...
    Commands.prototype._sampleKeymapFile = function() {
      return {
