...
STRAP_GIT_EMAIL='colin@github.com'
STRAP_GITHUB_USER='lildude'
STRAP_GITHUB_TOKEN=''
STRAP_ISSUES_URL='https://github.com/github/strap-internal/issues/new'
...
# Download dotfiles
if [ -n "$STRAP_GITHUB_USER" ]; then
  REPO_URL="https://github.com/$STRAP_GITHUB_USER/dotfiles"
  STATUS_CODE=$(curl --silent --write-out "%{http_code}" --output /dev/null $REPO_URL/blob/HEAD/README.md)
  if [ "$STATUS_CODE" -eq 200 ]; then
    logn "Fetching user dotfiles repo from GitHub:"
    if [ ! -d "$HOME/.dotfiles" ]; then
      git clone --recursive $Q $REPO_URL ~/.dotfiles
    else
      cd ~/.dotfiles
      git pull $Q
    fi
